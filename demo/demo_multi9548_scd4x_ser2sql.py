...
# Streamlit 应用
st.title('Multiple I2C Multiplexer + SCD4x Visualization')
...
# 获取所有唯一的传感器编号
query_id = "SELECT DISTINCT id FROM multi9548_scd4x"
cursor = db_connection.cursor()
cursor.execute(query_id)
sensor_ids = cursor.fetchall()
sensor_ids = [str(id[0]) for id in sensor_ids]  # 提取编号列表

# 创建一个侧边栏
st.sidebar.title('Options')

default_sensor_ids = sensor_ids  # 默认选择传感器ID
...
# 构建由多个OR连接的条件语句
conditions = " OR ".join([f"id = ?" for _ in selected_sensor_ids])
selected_id_list = list(selected_sensor_ids)
...
FROM multi9548_scd4x
WHERE id IN {tuple(selected_id_list)}
...
st.sidebar.download_button(
    ...
    data=download_data(),
    file_name="mutli9548_scd4x_trial_data.csv",
    mime='text/csv'
)
...
if not df.empty:
    ...
    for n in range(len(selected_sensor_ids)):
        sensor_data = df[df['id'] == selected_sensor_ids[n]]
        sensor_data.plot(x='datetime', y='CO2', kind='scatter', s=3, c=palette[n], ax=ax[0], label=f'Sensor {selected_sensor_ids[0]}')
        sensor_data.plot(x='datetime', y='temperature', kind='scatter', s=3, c=palette[n], ax=ax[1], label=f'Sensor {selected_sensor_ids[0]}')
        sensor_data.plot(x='datetime', y='humidity', kind='scatter', c=palette[n], s=3, ax=ax[2], label=f'Sensor {selected_sensor_ids[0]}')
